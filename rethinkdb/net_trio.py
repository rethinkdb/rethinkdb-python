...
PbResponse = ql2_pb2.Response
PbQuery = ql2_pb2.Query
...
class TrioFuture:
    ...
    def exception(self):
        if self._event.is_set():
            return self._exc
        raise RuntimeError("Future value has not been set")
...
class TrioCursor(Cursor, trio.abc.AsyncResource):
    ...
    async def close(self):  # pylint: disable=invalid-overridden-method
        ...
        if self.error is None:
            ...
            if self.conn.is_open():
                ...
                await self.conn._parent._stop(self)  # pylint: disable=protected-access
    ...
    def _empty_error(self):  # pylint: disable=arguments-differ
...
class ConnectionInstance:
    ...
    async def run_query(self, query, noreply):
        await self._send(query.serialize(self._parent.get_json_encoder(query)))
        if noreply:
            return None
...
class Connection(ConnectionBase):
    def __init__(self, *args, **kwargs):
        self._nursery = kwargs.pop("nursery", None)
    ...
    async def reconnect(
        ...
        self._instance = self._conn_type(
            self, nursery=self._nursery, **self._child_kwargs
        )
...
# Note: No monkeypatching into the global RethinkDB object.
