    ...
    def __call__(
        ...
        if not isinstance(query_str, ast.RqlQuery):
            raise AssertionError(
                f"Query must be a ReQL query instead of {format(value=query_str)}"
            )

        if not isinstance(times, int) or times < 1:
            raise ValueError(
                f"Times must be a positive integer instead of {format(value=times)}"
            )
        ...
        if not isinstance(run_options, dict):
            raise ValueError(
                f"Run option must be a dict instead of {format(value=run_options)}"
            )
        ...
        for _ in range(times):
            try:
                ...
                last_error = RuntimeError(
                    f"Error connecting for during '{name}': {str(e)}"
                )
            ...
            try:
                ...
                last_error = RuntimeError(
                    f"Connection error during '{name}': {str(e)}"
                )
...
def print_progress(ratio, indent=0, read=None, write=None):
    ...
    sys.stdout.write(
        f"\r{' ' * indent}[{'=' * done_width}"
        f"{' ' * (total_width - done_width)}] "
        f"{int(100 * ratio):.3f}{(f' r: {read}') if read is not None else ''}"
        f"{(f' w: {write}') if write is not None else ''}\x1b[K"
    )
    sys.stdout.flush()
...
def check_minimum_version(options, minimum_version="1.6", raise_exception=True):
    ...
    if not matches:
        raise RuntimeError(f"invalid version string format: {version_string}")

    if distutils.version.LooseVersion(matches.group("version")) < minimum_version:
        if raise_exception:
            raise RuntimeError(
                f"Incompatible version, expected >= "
                f"{minimum_version} got: {version_string}"
            )
        return False
    return True
...
class CommonOptionsParser(optparse.OptionParser, object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def check_tls_option(_, opt_str, value):
            ...
            if os.path.isfile(value):
                ...
                raise optparse.OptionValueError(
                    f"Option {opt_str} value is not a file: {value}"
                )
        ...
        def check_db_table_option(_, _opt_str, value):
            ...
            if not res:
                raise optparse.OptionValueError(
                    f"Invalid db or db.table name: {value}"
                )
        ...
        def check_positive_int(_, opt_str, value):
            try:
                ...
                raise optparse.OptionValueError(
                    f"{opt_str} value must be an integer greater than 1: {value}"
                )
        ...
        def check_existing_file(_, opt_str, value):
            if not os.path.isfile(value):
                raise optparse.OptionValueError(
                    f"{opt_str} value was not an existing file: {value}"
                )
        ...
        def check_new_file_location(_, opt_str, value):
            try:
                ...
                raise optparse.OptionValueError(
                    f"Incorrect value for {opt_str}: {value}"
                )

            if os.path.exists(real_value):
                raise optparse.OptionValueError(
                    f"{opt_str} value already exists: {value}"
                )
        ...
        def file_contents(_, opt_str, value):
            if not os.path.isfile(value):
                raise optparse.OptionValueError(
                    f"{opt_str} value is not an existing file: {value}"
                )
        ...
        def combined_connect_action(obj, opt, value, parser, *args, **kwargs):
            ...
            if not res:
                raise optparse.OptionValueError(
                    f"Invalid 'host:port' format: {value}"
                )
        ...
        class CommonOptionChecker(optparse.Option, object):
            ...
            def take_action(self, action, dest, opt, value, values, parser):
                ...
                if action == "add_key":
                    ...
                    values.ensure_value(
                        "password",
                        getpass.getpass("Password for `admin`: ")
                    )
                else:
        ...
        connection_group.add_option(
            ...
            metavar="HOST:PORT",
            help=(
                f"host and client port of a rethinkdb node to connect "
                f"(default: localhost:{net.DEFAULT_PORT})"
            ),
            action="callback",
    ...
    def parse_args(self, *args, **kwargs):
        ...
        if "RETHINKDB_DRIVER_PORT" in os.environ:
            ...
            if not isinstance(driver_port, int) or driver_port < 1:
                self.error(
                    ...
                    f"integer: {os.environ['RETHINKDB_DRIVER_PORT']}"
                )
        ...
        if connect:
            try:
                ...
                self.error(f"Unable to connect to server: {str(e)}")
