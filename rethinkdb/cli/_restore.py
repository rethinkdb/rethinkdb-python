...
def parse_options(argv, prog=None):
    ...
    if options.in_file == "-":
        ...
        if not os.path.isfile(options.in_file):
            parser.error(f"Archive file does not exist: {options.in_file}")
    ...
    try:
        ...
        for tarinfo in archive:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Error: Archive file has an unexpected directory structure: {tarinfo.name}"
                )
            ...
            if not top_level:
                ...
                raise RuntimeError(
                    f"Error: Archive file has an unexpected directory structure ({top} vs {top_level})"
                )
            ...
            if not os.path.isfile(dest_path):
                raise AssertionError(
                    f"Was not able to write {dest_path}"
                )
