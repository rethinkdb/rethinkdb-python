...
def parse_options(argv, prog=None):
    if platform.system() == "Windows" or platform.system().lower().startswith("cygwin"):
        # no colons in name
        default_dir = f'''rethinkdb_export_{datetime.datetime.today().strftime(
            "%Y-%m-%dT%H-%M-%S")}
        '''
    else:
        default_dir = f'''rethinkdb_export_{datetime.datetime.today().strftime(
    ...
    if options.format == "csv":
        ...
        if options.delimiter is None:
            ...
            parser.error(
                f"Specify exactly one character for the --delimiter option: {options.delimiter}"
            )
    else:
    ...
    try:
        ...
        if has_write_hooks:
            table_info["write_hook"] = options.retryQuery(
                f"table write hook data {db}.{table}",
        ...
        if options.format == "json":
            filename = directory + f"/{db}/{table}.json" % (db, table)
            ...
            filename = directory + f"/{db}/{table}.csv" % (db, table)
            writer = multiprocessing.Process(
                target=csv_writer,
                args=(
                    filename,
            ...
            filename = directory + f"/{db}/{table}.ndjson" % (db, table)
            writer = multiprocessing.Process(
                target=json_writer,
                args=(
                    filename,
            ...
            raise RuntimeError(f"unknown format type: {options.format}")
        writer.start()
...
def run_clients(options, workingDir, db_table_set):
    ...
    if len(errors) != 0:
        ...
        for error in errors:
            print(f"{error[1]}", file=sys.stderr)
            if options.debug:
                print(
                    f"{error[0].__name__} traceback: {error[2]}", file=sys.stderr
                )
...
def run(options):
    ...
    if not options.db_tables:
        ...
        for db_table in options.db_tables:
            ...
            if db not in all_databases:
                raise RuntimeError(f"Error: Database '{db}' not found")
    ...
    if not os.path.exists(parent_dir):
        if os.path.isdir(parent_dir):
            raise RuntimeError(
                f"Output parent directory is not a directory: {parent_dir}"
            )
        try:
            os.makedirs(parent_dir)
        except OSError as e:
            raise optparse.OptionValueError(
                f"Unable to create parent directory for {parent_dir}: {e.strerror}"
            )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to create temporary directory ({e.filename}): {e.strerror}"
        )
    ...
    try:
        if os.path.isdir(options.directory):
            ...
            raise Exception(
                f"There was a file at the output location: {options.directory}"
            )
