...
usage = (
    ...
    "[-r (DB | DB.TABLE)] [--tls-cert FILENAME] [-p] "
...
def parse_options(argv, prog=None):
    ...
    if len(args) != 0:
        parser.error(
            f"Error: No positional arguments supported. "
            f"Unrecognized option '{args[0]}'"
        )
...
def rebuild_indexes(options):
    ...
    for db, table in options.db_table:
        ...
        if not options.force:
            indexes = options.retryQuery(
                f"get outdated indexes from {db}.{table}" % (db, table),
                query.db(db)
                .table(table)
                .index_status()
            ...
            indexes = options.retryQuery(
                f"get all indexes from {db}.{table}",
    ...
    if not options.quiet:
        print(
            f"Rebuilding {total_indexes} index"
            f"{'es' if total_indexes > 1 else ''}: "
            f", ".join(
                [
                    f"`%({i.db})s.%({i.table})s:%({i.name})s`"
    ...
    while len(indexes_to_build) > 0 or len(indexes_in_progress) > 0:
        ...
        while (
            ...
            if index["name"] not in existing_indexes:
                raise AssertionError(
                    f"{index['name']} is not part of existing indexes "
                    f"{', '.join(existing_indexes)}"
                )
        ...
        for index in indexes_in_progress:
            status = options.retryQuery(
                f"progress `{index.db}s.{index.table}s` "
                f"index `{index.name}s`",
                query.db(index["db"])
                .table(index["table"])
            ...
            if status["ready"]:
                index["ready"] = True
                options.retryQuery(
                    f"rename `{index.db}s.{index.table}s` "
                    f"index `{index.name}s`",
                    query.db(index["db"])
                    .table(index["table"])
