    ...
    def restore_indexes(self, warning_queue):
        ...
        if self.indexes:
            existing_indexes = self.query_runner(
                f"indexes from: {self.db}.{self.table}",
            ...
            try:
                ...
                self.query_runner(
                    f"waiting for indexes on {self.db}.{self.table}",
                    query.db(self.db)
                    .table(self.table)
        ...
        if self.write_hook:
            self.query_runner(
                f"Write hook from: {self.db}.{self.table}",
                query.db(self.db).table(self.table).get_write_hook(),
            )
            try:
                self.query_runner(
                    f"drop hook: {self.db}.{self.table}",
                    query.db(self.db).table(self.table).set_write_hook(None),
                )
                self.query_runner(
                    f"create hook: {self.db}.{self.table}:{self.write_hook}",
                    query.db(self.db)
                    .table(self.table)
...
def parse_options(argv, prog=None):
    ...
    if options.directory and options.file:
        ...
        if options.format == "csv":
            ...
            parser.error(f"Unrecognized file format: {options.format}")
...
def import_tables(options, sources, files_ignored=None):
    ...
    for source in sources:
        if (source.db, source.table) in existing_tables:
            if not options.force:
                already_exist.append(f"{source.db}.{source.table}")
            ...
            while not exit_event.is_set():
                ...
                while len(readers) < options.clients:
                    ...
                    reader = multiprocessing.Process(
                        ...
                        name=f"table reader {table.db}.{table.table}",
                        kwargs={
    ...
    for error in errors:
        ...
        if len(error.file) == 4:
            print(f"  In file: {error.file}", file=sys.stderr)

    for warning in warnings:
        print(f"{warning[1]}", file=sys.stderr)
        if options.debug:
            print(
                f"{warning[0].__name__, warning[2]} traceback: {sys.stderr}")
        if len(warning) == 4:
            print(f"In file: {warning[3], sys.stderr}")
                ...
                for filename in files:
                    ...
                    if ext not in [".json", ".csv", ".info"]:
                        ...
                        if ext == ".json":
                            ...
                            raise Exception(
                                f"The table type is not recognised: {ext}"
                            )
                        source = table_type(
                            source=path,
                            query_runner=options.retryQuery,
