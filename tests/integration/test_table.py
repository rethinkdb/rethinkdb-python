...
from rethinkdb.errors import ReqlRuntimeError, ReqlOpFailedError
...
class TestTable(IntegrationTestCaseBase):
    ...
    def test_table_different_primary_key(self):
        ...
        assert result['tables_created'] == 1
        assert len(result['config_changes']) == 1
        assert result['config_changes'][0]['new_val']['primary_key'] == expected_primary_key
    ...
    def test_table_create_with_replicas(self):
        ...
        assert result['tables_created'] == 1
        assert len(result['config_changes']) == 1
        assert len(result['config_changes'][0]['new_val']['shards'][0]['replicas']) == expected_replicas
    ...
    def test_table_multiple_replicas(self):
        ...
        with pytest.raises(ReqlOpFailedError):
            self.r.table_create(self.test_table_name, replicas=expected_replicas).run(self.conn)
