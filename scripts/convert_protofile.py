    ...
    for line in inputFile:
        ...
        if match is not None:
            ...
            if levelHasContent:
                outputFile.write("\n" + "\t" * indentLevel)
            outputFile.write(
                languageDefs[language]["open"]
                % {"tabs": "\t" * indentLevel, "name": match.group("name")}
            )
        ...
        if match is not None:
            if not levelHasContent and languageDefs[language]["empty"] is not None:
                outputFile.write(
                    "\n" + "\t" * indentLevel + languageDefs[language]["empty"]
                )
                lastIndentLevel = indentLevel
            if languageDefs[language]["close"] is not None:
                if (
                    indentLevel == lastIndentLevel
                    or languageDefs[language]["closeAlwaysNewLine"] is True
                ):
                    outputFile.write("\n" + "\t" * (indentLevel - 1))
...
if __name__ == "__main__":
    ...
    if options.inputFile is not None:
        try:
            ...
            parser.error(
                "Unable to open the given input file <<%s>>"
    ...
    if options.outputFile is not None:
        try:
            ...
            parser.error(
                "Unable to open the given output file <<%s>>,"
